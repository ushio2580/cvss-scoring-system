...
interface EditEvaluationModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (evaluationId: number, data: any) => void;
...
export const EditEvaluationModal: React.FC<EditEvaluationModalProps> = ({
  ...
  // Load evaluation data when modal opens
  useEffect(() => {
    if (evaluation && isOpen) {
      ...
      // Build vector from metrics
      const vectorParts = ['CVSS:3.1'];
      baseMetrics.forEach(metric => {
        if (evaluation.metrics[metric.key]) {
          vectorParts.push(`${metric.key}:${evaluation.metrics[metric.key]}`);
        }
      });
      
      // Check if temporal metrics exist
      const hasTemporal = temporalMetrics.some(metric => evaluation.metrics[metric.key]);
      setShowAdvanced(hasTemporal);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onSubmit(evaluation.id, evaluationData);
    } catch (error) {
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="p-4 bg-muted/20 rounded-lg">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                      ...
                      <div>
                        <span className="font-medium">Author:</span> {evaluation.author_name}
                      </div>
                      <div>
